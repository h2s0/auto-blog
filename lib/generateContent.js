...
function isHtmlComplete(html) {
  // 마지막 의미 있는 내용이 닫힌 태그 또는 링크로 끝나야 함
  const trimmed = html.trimEnd();
  return /(<\/p>|<\/ul>|<\/li>|→<\/a><\/p>)\s*$/.test(trimmed);
}
  ...
  if (candidate.finishReason === 'MAX_TOKENS') {
    if (maxTokens > 8192) {
      ...
      const shorterPrompt = prompt
        .replace(/1800~2200자/, '1200~1500자')
        .replace(/5~7문장/g, '3~5문장')
        .replace(/1500~2000자/, '1000~1300자')
        .replace(/1000~1500자/, '700~1000자');
      return runGemini(shorterPrompt, 8192);
    }
