...
function buildExhibitionNewsPrompt(newsItems) {
  const today = new Date().toLocaleDateString('ko-KR', {
  ...
  const newsList = newsItems.map((n, i) => `[${i + 1}] ${n.title}
  출처: ${n.source || '알 수 없음'}
  발행일: ${n.pubDate || '알 수 없음'}
  원문: ${n.link}
  이미지: ${n.imageUrl || '없음'}`).join('\n\n');

  return `당신은 서울 전시·미술관 소식을 소개하는 블로그 에디터입니다.
오늘 날짜: ${today}

아래는 뉴스 검색으로 수집한 서울 전시 관련 기사 목록입니다. 이 중 실제로 "신규 전시 개막", "얼리버드·사전예매 티켓 오픈", "미술관/갤러리 개관예정" 등 독자가 놓치면 아까울 시의성 있는 소식만 골라 정리하는 블로그 포스트를 작성해주세요.

[선별 기준]
- 서울(수도권 포함) 미술관·갤러리 관련 소식만. 지방 단독 전시, 관련 없는 기사(부고·정치·지역축제 등)는 제외.
- 아래 성격에 해당하는 것만 채택:
  * 신규 전시 개막/오픈 소식
  * 얼리버드·사전예매 티켓 오픈
  * 미술관/갤러리 신규 개관 예정
- 채택할 소식이 부족하면 무리하게 채우지 말고 있는 만큼만 다룰 것.
- 제목만으로 알 수 없는 세부 정보(정확한 가격·기간 등)는 지어내지 말고 "원문 참고"로 처리.

[HTML 구조]
1. <p> 도입부 </p> — 요즘 서울에서 화제인 전시 소식이 있다는 내용 2~3문장
2. 채택한 소식마다:
   <h2>[번호]. [전시/미술관명] — [한 줄 요약]</h2>
   (이미지가 "없음"이 아니면 <h2> 바로 다음 줄에 <p><img src="[이미지 URL]" alt="[전시/미술관명]" style="max-width:100%;height:auto;border-radius:8px;" /></p> 추가. 이미지가 "없음"이면 생략.)
   <p> 무슨 소식인지, 왜 주목할 만한지 2~3문장 </p>
...
- 어투: 20~30대 구어체 "-요" 체. "~입니다" 문어체 금지. AI 티 나는 뻔한 문장 금지.
- 줄바꿈: <p> 태그 하나에 1~2문장만. 문장이 3개 이상이면 반드시 <p>를 나눌 것. 모바일 가독성 최우선.
- HTML 태그만: h2, p, ul, li, strong, a, img
- 이미지 URL은 [수집된 뉴스 목록]에 주어진 값을 절대 변형하지 말고 그대로 사용. 새로 지어내지 말 것.
- title: 아무 값이나 넣어도 됨 (코드에서 덮어씀)
- labels — 실제 검색어 형태로 6~8개:
  * "서울 전시", "서울 전시회 추천", "서울 미술관 전시", "얼리버드 티켓" 중 다수 포함
  * "서울 볼거리", "서울 놀거리", "서울 가볼만한곳" 중 2개 이상 포함
  * 언급된 작가명·미술관명이 있으면 "[작가명] 전시"처럼 구체적으로 포함
...
export async function generatePostForExhibitionNews(newsItems) {
  console.log(`[generateContent] 전시 소식 포스트 생성 중...`);
  const post = await runGemini(buildExhibitionNewsPrompt(newsItems));
  if (!post.html || !Array.isArray(post.labels)) {
    throw new Error(`필수 필드 누락: ${JSON.stringify(post)}`);
  }

  // 제목 형식 강제 적용 ("전시 소식" 마커 포함 — 당일 중복 발행 방지용 키워드로도 사용됨)
  const [year, month, day] = getKstDateString().split('-').map(Number);
  post.title = `${year}년 ${month}월 서울 전시 소식 - 신규 개막·얼리버드 티켓·개관예정 총정리 (${month}/${day})`;
