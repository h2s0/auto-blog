...
function buildParkingSection(parkingLots) {
  ...
  const items = parkingLots
    ...
    .join('\n');
  return `\n[실제 근처 주차장 데이터 — 반드시 이 내용을 🚗 주차 안내 섹션에 포함]\n${items}`;
}

function buildPrompt(event, scrapedText, parkingLots) {
  ...
  const extraInfo = scrapedText
    ? `\n[공식 페이지 수집 정보]\n${scrapedText}`
    : '';
  const parkingInfo = buildParkingSection(parkingLots);
...
function parseResponse(rawText) {
  ...
  // 3차: JSON이 잘렸을 때 html 필드만 닫고 복구 시도
  ...
  if (titleMatch && htmlMatch) {
    const html = htmlMatch[1].replace(/"[\s\S]*$/, '').replace(/\\n/g, '\n');
    return {
      title: titleMatch[1],
      html,
      labels: labelsMatch ? JSON.parse(labelsMatch[1]) : ['서울', '문화행사'],
    };
  }
...
export async function generatePostForEvent(event, scrapedText = null, parkingLots = []) {
  ...
  const body = {
    contents: [{ parts: [{ text: buildPrompt(event, scrapedText, parkingLots) }] }],
    generationConfig: {
