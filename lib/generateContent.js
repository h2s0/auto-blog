...
아래 정부 정책 1건을 서울 시민 입장에서 깊고 실용적으로 설명하는 블로그 포스트를 작성해주세요.
"나는 해당되나? 어떻게 신청하나?"에 집중하고, 어려운 행정 용어는 모두 일상 언어로 바꾸세요.

[HTML 구조]
1. <p> 도입부 </p> — 이 정책이 왜 중요한지, 어떤 사람에게 해당되는지 2~3문장

2. <h2>💡 [정책명을 쉬운 말로]</h2>

3. <h2>📋 대상 & 혜택</h2>
   <ul>
     <li><strong>대상:</strong> 구체적으로 어떤 사람 (나이·조건 등 명시)</li>
     <li><strong>혜택 내용:</strong> 금액·기간·방식 등 구체적으로</li>
     <li><strong>언제부터:</strong> 시행 시기 또는 신청 기간</li>
   </ul>

4. <h2>✅ 신청 방법</h2>
   <ul>
     <li>단계별로 어떻게 신청하는지 실제 절차 서술 (온라인이면 어느 사이트, 오프라인이면 어디로)</li>
     <li>필요 서류가 있다면 언급</li>
     <li><a href="[원문URL]" target="_blank" rel="noopener">공식 안내 바로가기 →</a></li>
   </ul>

5. <h2>⚠️ 주의사항</h2>
   <ul> 놓치기 쉬운 조건이나 기한 1~2개 </ul>
...
- "원문 확인 필요" 금지 — 알 수 있는 정보는 직접 서술, 정말 모르는 것만 "공식 사이트 확인"으로 처리
- 없는 정보를 지어내지 말 것 (금액·날짜를 모르면 범위나 "공고 예정"으로)
- HTML 태그: h2, p, ul, li, strong, a
- title: "[정책명] 신청 방법과 대상 정리" 형식 (날짜 넣지 말 것)
- labels — 실제 검색어 6~8개:
  * "서울 복지 혜택", "정부 지원금", "서울 시민 혜택" 중 2개 이상
  * 주제별 구체적 검색어
...
4. <h2>✅ ${topic} 핵심 내용</h2>
   - 주제에 두 가지 이상 항목을 비교하는 내용이 포함된 경우 (예: A vs B, A와 B의 차이, 유형별 비교 등)
     반드시 아래 형식의 HTML 표를 사용하세요:
     <table style="width:100%;border-collapse:collapse;margin:1em 0;">
       <thead>
         <tr style="background:#f0f4ff;">
           <th style="border:1px solid #dde;padding:10px;text-align:left;">구분</th>
           <th style="border:1px solid #dde;padding:10px;text-align:left;">항목A</th>
           <th style="border:1px solid #dde;padding:10px;text-align:left;">항목B</th>
         </tr>
       </thead>
       <tbody>
         <tr>
           <td style="border:1px solid #dde;padding:10px;">대상</td>
           <td style="border:1px solid #dde;padding:10px;">...</td>
           <td style="border:1px solid #dde;padding:10px;">...</td>
         </tr>
         ... (필요한 행만큼)
       </tbody>
     </table>
   - 비교 내용이 없는 경우: 소주제 1~2개를 <h3>로 구분해 서술.
...
[작성 규칙]
- 분량: 전체 800~1100자 (표 포함 시 1100자까지 허용. 그 외 절대 1000자 초과 금지)
- 어투: 20~30대 구어체 "-요" 체. "~입니다" 문어체 금지.
- 줄바꿈: <p> 태그 하나에 1~2문장만.
- HTML 태그: h2, h3, p, ul, li, strong, a, table, thead, tbody, tr, th, td 사용 가능. style 속성은 위 지정된 것만.
- 없는 정보 지어내지 말 것.
- labels — 실제 검색어 형태로 6~8개:
...
export async function generatePostForLhNotice(notices) {
  ...
  const year = now.getFullYear();
  const month = now.getMonth() + 1;
  post.title = `LH·SH 청약 공고 총정리 (${year}.${String(month).padStart(2,'0')}) - 공공임대·분양 신청 정보`;
