 ...
 * 1:1 (800×800) 정사각형 — Blogger 목록 크롭에 최적화
 */
 ...
 * @param {boolean|null} [opts.isFree]
...
export function generateThumbnailSvg({ title, subtitle, category, district, isFree } = {}) {
  ...
  const titleStartY = Math.round((H - blockH) / 2) + 20;

  const freeLabel = isFree === true ? '무료' : isFree === false ? '유료' : null;
  const freeBgColor = isFree ? '#10B981' : '#6B7280';

  const catText = esc(category || '서울로그');
  const catBadgeW = Math.max(catText.length * 16 + 48, 90);

  const subText = [district, subtitle].filter(Boolean).join(' · ');
  const subY = titleStartY + lines.length * lineH + 32;

  const titleSvg = lines
    .map(
      (line, i) =>
        `<text x="${CX}" y="${titleStartY + i * lineH}"
      font-family="'Apple SD Gothic Neo','Noto Sans KR','Malgun Gothic',sans-serif"
      font-size="62" font-weight="800" fill="white" letter-spacing="-1.5"
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <circle cx="${W}" cy="0"   r="360" fill="white" fill-opacity="0.05"/>
  <circle cx="0"   cy="${H}" r="260" fill="white" fill-opacity="0.04"/>
  <circle cx="${CX}" cy="${Math.round(H * 0.72)}" r="130" fill="${theme.accent}" fill-opacity="0.13"/>
  <rect x="0" y="0" width="7" height="${H}" fill="${theme.accent}"/>
  <rect x="${Math.round(CX - catBadgeW / 2)}" y="48" width="${catBadgeW}" height="46" rx="23"
        fill="${theme.accent}" fill-opacity="0.22"/>
  <text x="${CX}" y="80"
        font-family="'Apple SD Gothic Neo','Noto Sans KR',sans-serif"
        font-size="21" font-weight="600" fill="${theme.accent}"
        text-anchor="middle">${catText}</text>
  ${freeLabel ? `<rect x="${W - 110}" y="48" width="78" height="46" rx="23" fill="${freeBgColor}" fill-opacity="0.92"/>
  <text x="${W - 71}" y="80"
        font-family="'Apple SD Gothic Neo','Noto Sans KR',sans-serif"
        font-size="21" font-weight="700" fill="white" text-anchor="middle">${esc(freeLabel)}</text>` : ''}
  ${titleSvg}
  ${subText ? `<text x="${CX}" y="${subY}"
      font-family="'Apple SD Gothic Neo','Noto Sans KR',sans-serif"
      font-size="26" font-weight="400" fill="rgba(255,255,255,0.58)"
      text-anchor="middle">${esc(subText)}</text>` : ''}
  <rect x="40" y="${H - 56}" width="${W - 80}" height="1" fill="white" fill-opacity="0.18"/>
  <text x="${CX}" y="${H - 24}"
        font-family="'Apple SD Gothic Neo','Noto Sans KR',sans-serif"
        font-size="20" font-weight="300" fill="rgba(255,255,255,0.42)"
        text-anchor="middle">서울로그</text>
</svg>`;
