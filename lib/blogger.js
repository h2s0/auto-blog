...
export async function fetchTodayPosts() {
  ...
  const url = `${BLOGGER_API}/${blogId}/posts?maxResults=10&status=live&fetchBodies=false`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) {
    console.log(`[blogger] 오늘 포스트 조회 실패 (${res.status}) — 중복 방지 건너뜀`);
    return [];
  }

  const data = await res.json();
  const items = data.items ?? [];
  const todayPosts = items.filter((p) => {
  ...
  console.log(`[blogger] 오늘 발행된 포스트: ${todayPosts.length}개`);
  return todayPosts;
}
