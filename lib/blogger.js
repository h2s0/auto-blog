 ...
 * 현재는 isDraft=true (초안 저장). 검수 후 false로 변경할 것.
 */
...
export async function publishPost({ title, html, labels }) {
  ...
  console.log(`  URL: ${post.url ?? '(초안 — 미발행)'}`);
  // 초안일 경우 Blogger 편집 링크 추가
  post.editUrl = `https://www.blogger.com/blog/post/edit/${blogId}/${post.id}`;
  return post;
}
