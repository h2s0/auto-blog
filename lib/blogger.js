...
export async function fetchRecentTitles(days = 7) {
  ...
  const fields = 'items(title,published)';

  async function fetchByStatus(status) {
    const url = `${BLOGGER_API}/${blogId}/posts?maxResults=50&status=${status}&fetchBodies=false&startDate=${startIso}&fields=${fields}`;
  ...
  const [liveItems, scheduledItems] = await Promise.all([
    fetchByStatus('live'),
    fetchByStatus('scheduled'),
  ]);

  const all = [...liveItems, ...scheduledItems];
  console.log(`[blogger] 최근 ${days}일 포스트: live=${liveItems.length} + scheduled=${scheduledItems.length}건`);
