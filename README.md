...
서울의 문화행사·무료공간·정책뉴스·전시 소식을 매일 수집하여 Gemini AI로 블로그 글을 생성하고 Blogger에 자동 발행하는 파이프라인.

**GitHub Actions가 매일 KST 09/12/15/18시에 실행. 서버 상시 구동 없음. 전 구성요소 무료 티어.**

## 아키텍처

```
GitHub Actions (매일 KST 09/12/15/18시)
    → 서울 열린데이터광장 API / 구글 뉴스 RSS 등 (오늘의 소식 수집)
    → Gemini API (블로그 원고 생성)
...
| 시각 | 요일 | 파이프라인 |
|---|---|---|
| 09:00 | 월/수/금/일 | 오늘의 서울 문화행사 |
| 09:00 | 화/목 | 구별 무료 문화 프로그램 모음 |
| 09:00 | 토 | 오늘의 정책뉴스 |
| 12:00, 15:00 | 월/수/금/일만 | 오늘의 서울 문화행사 (추가 2건) |
| **18:00** | **매일** | **서울 전시 소식** (신규 개막·얼리버드 티켓·개관예정, 구글 뉴스 RSS 기반) |

화/목/토는 09:00에 1건만 발행하도록 설계되어 있어(`index.js`의 "하루 1회" 로직), 하루 총 발행량은 요일에 따라 2~4건입니다.
